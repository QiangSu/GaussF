...
The kmer_frequency_distribution_loop.py script is designed to analyze transcript sequences at the isoform level to determine the frequency of each k-mer (where k is a fixed length). The script calculates both the local frequency of k-mers within individual transcripts (isoform-specific) and the global frequency across all analyzed transcripts, highlighting the complexity of isoform overlapping regions. The local frequency reflects the occurrence of each k-mer within a specific isoform, while the global frequency provides insight into the commonality of each k-mer across all isoforms. The output of the script can be used to study transcript diversity, uncover patterns of sequence conservation, and examine potential isoform overlaps.
